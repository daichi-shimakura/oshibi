...
C#/EF Coreでいう **TPT（Table per Type）継承** と同じ発想です。FANとLIVE_STAFFで共通の属性（表示名・SNSリンク等）は `accounts` に、芸人特有の属性（ユニット種別・事務所等）だけを `comedian_profiles` に切り出しています。
...
### 5. @EntityGraph による N+1 問題の解消

ライブ一覧で出演者情報を表示するため、`@EntityGraph` で `live_performers` を JOIN FETCH。N+1 問題を一発で回避しています（C# でいう `Include()` と同じ発想）。
...
erDiagram
    users ||--|| accounts : "has"
    accounts ||--o| comedian_profiles : "extends (LIVE_STAFF only)"
    ...
    accounts {
        ...
        bigint user_id FK
        string account_type "FAN / LIVE_STAFF"
        string display_name
        text description
    ...
    comedian_profiles {
        ...
        string unit_type "PIN/COMBI/TRIO/GROUP/STAFF"
        string agency
        text member_names
    }
    venues {
...
> **`comedian_profiles` のPKは独立カラムではなく `account_id` そのもの**（識別関係 / Identifying Relationship）です。これにより、芸人プロフィールはアカウントと1:1で必ず紐付き、孤児レコードが物理的に作れない構造になっています。詳細は[技術的なこだわり](#技術的なこだわり)を参照。
